...
/*
 * 窗户拼字：一个「国」字，25 格宽 x 16 格高。
 *
 * 这里刻意不放"中国地图轮廓"。地图（国界线、台湾岛、南海诸岛）必须严格
 * 符合国家标准，用几十个窗户格去手工近似国界，既不可能画准、观感也很差，
 * 所以改成拼一个不涉及疆域的字——「国」，题眼仍然是「代码寄家国」。
 *
 * 格子的实际像素是 15.31 宽 x 24.3 高，并不方正，所以位图按 15.31:24.3
 * 的长宽比排版，字落到屏幕上才是正的、没有被拉扁。
 */
const char *kGuoBitmap[] = {
    "000000000000000000000000000000",
    "000000000000000000000000000000",
    "000011111111111111111111110000",
    "000011100000000000000001110000",
    "000011100000000000000001110000",
    "000011100111111111111101110000",
    "000011100000001100000001110000",
    "000011100000001100000001110000",
    "000011100111111111111001110000",
    "000011100000001100000001110000",
    "000011100000001101110001110000",
    "000011100000001100000001110000",
    "000011101111111111111101110000",
    "000011100000000000000001110000",
    "000011100000000000000001110000",
    "000011111111111111111111110000",
...
constexpr int kGuoW = 30;
constexpr int kGuoH = 19;
...
void GameWidget::buildPatterns()
{
    ...
    /* 一个「国」字：把 25x16 的窗格位图居中铺到 64x20 的网格上。
     * 地图不让手工近似（国界必须严格符合国家标准），改拼字更稳、更好看。 */
    m_guoCells.clear();
    {
        const int startCol = (kCols - kGuoW) / 2;
        const int startRow = (kRows - kGuoH) / 2;
        for (int r = 0; r < kGuoH; ++r) {
            const char *line = kGuoBitmap[r];
            for (int c = 0; c < kGuoW; ++c) {
                if (line[c] != '1')
                    continue;
                m_guoCells << Cell{ startCol + c, startRow + r };
            }
        }
    }

    // 以字心为原点按角度排序，点亮时像一轮日光扫过（从中心一圈圈推开）
    QPointF sum(0.0, 0.0);
    for (const Cell &c : m_guoCells)
        sum += QPointF(c.col, c.row);
    const QPointF mid = !m_guoCells.isEmpty() ? sum / qreal(m_guoCells.size()) : QPointF();
    std::sort(m_guoCells.begin(), m_guoCells.end(), [mid](const Cell &a, const Cell &b) {
...
void GameWidget::tick()
{
    ...
    if (m_phase == Playing) {
        ...
        if (m_sevenIndex >= m_sevenCells.size() && m_phaseTime > 2.6 && !m_demoHold) {
            m_phase = MorphGuo;
            m_phaseTime = 0.0;
            m_revealAcc = 0.0;
    ...
    } else if (m_phase == MorphGuo) {
        m_phaseTime += dt;
        const qreal rate = qreal(m_guoCells.size()) / 2.6;      // 2.6 秒点亮整个字
        m_revealAcc += dt * rate;
        while (m_revealAcc >= 1.0 && m_guoIndex < m_guoCells.size()) {
            const Cell &c = m_guoCells.at(m_guoIndex++);
            const int idx = c.row * kCols + c.col;
            m_cellTarget[idx] = 1.0;
            m_cellPop[idx] = 1.0;
            m_revealAcc -= 1.0;
        }
        if (m_guoIndex >= m_guoCells.size() && m_phaseTime > 3.6 && !m_demoHold) {
            m_phase = Finished;
            m_phaseTime = 0.0;
        }
    } else {
...
/* 演示 / 截图专用：跳到窗户点阵的某一幕并全部点亮，随后冻结自动推进。
   main.cpp 的 --shots 用它抓「77」与「国」两张终局图。 */
void GameWidget::demoForcePhase(int phase)
{
    if (phase <= 0)
        return;
    ...
    // 视为六个词全部集齐
    std::fill(m_wordCount.begin(), m_wordCount.end(), m_needPerWord);
    m_caught = m_words.size() * m_needPerWord;
    m_cakes.clear();

    const QVector<Cell> &pattern = (phase == 1) ? m_sevenCells : m_guoCells;
    m_phase = (phase == 1) ? RevealSeven : MorphGuo;
    m_phaseTime = 0.0;
    m_revealAcc = 0.0;
    m_sevenIndex = m_sevenCells.size();
    m_guoIndex = m_guoCells.size();
    m_demoHold = true;                     // 冻结自动推进，留出抓图时间

    std::fill(m_cellTarget.begin(), m_cellTarget.end(), 0.0);
    for (const Cell &c : pattern) {
        const int idx = c.row * kCols + c.col;
        m_cellTarget[idx] = 1.0;
        m_cellBright[idx] = 1.0;
        m_cellPop[idx] = 0.0;
    }
    update();
}
...
void GameWidget::ensureCache()
{
    ...
    // 月亮：与其余场景共用同一张 images/moon.png
    const QPointF moon(1108.0, 118.0);
    const qreal moonR = 52.0;
    ...
    // 地面：铺一段实拍的楼前广场（有照片时用照片，否则退回纯色渐变）
    if (!m_bgImg.isNull()) {
        const QRectF plaza(0.0, 556.0, qreal(Theme::kSceneW), qreal(Theme::kSceneH) - 556.0);
        p.drawPixmap(plaza, m_bgImg, Theme::coverSourceRect(m_bgImg.size(), plaza));
        QLinearGradient shade(plaza.topLeft(), plaza.bottomLeft());
        shade.setColorAt(0.0, QColor(8, 11, 22, 176));
        shade.setColorAt(1.0, QColor(6, 8, 18, 214));
        p.fillRect(plaza, shade);
    } else {
    ...
    p.drawRect(QRectF(kFacade.left() + 300.0, kFacade.top() + 6.0, 420.0, 34.0));
    p.setFont(Theme::font(17, true));
    p.setPen(Theme::withAlpha(Theme::gold(), 226));
    p.drawText(QRectF(kFacade.left() + 300.0, kFacade.top() + 6.0, 420.0, 34.0),
               Qt::AlignCenter, QStringLiteral("软件学院 · 今夜为你亮灯"));
...
void GameWidget::drawHud(QPainter *p) const
{
    ...
    if (m_phase == Playing)
        ...
        caption = QStringLiteral("留校的窗，拼出「77」—— 祖国七十七华诞快乐");
    else if (m_phase == MorphGuo)
        caption = QStringLiteral("万家灯火，再拼出一个「国」");
    else
        caption = QStringLiteral("代码寄家国，月色赴华章");

    p->setFont(Theme::font(22, true));
    Theme::drawGlowText(p, QPointF(qreal(Theme::kSceneW) / 2.0, 612.0), caption,
                        Theme::cream(), Theme::gold(), 6.0);

    /* 底部说明：托盘在 y = 646..672 之间随鼠标横移，
     * 所以这行说明必须整体压到托盘下方（原来那句 "Esc 返回主场景" 画在
     * y = 668，正好被托盘压住，截图上看起来是一团糊字）。 */
    p->setFont(Theme::font(12));
    p->setPen(Theme::withAlpha(Theme::cream(), 170));
    p->drawText(QRectF(0.0, 700.0, qreal(Theme::kSceneW), 18.0), Qt::AlignCenter,
                QStringLiteral("每接住一个月饼，就点亮一扇留校的窗 · %1　｜　Esc 返回主场景 · R 重新开始")
                    .arg(Theme::schoolName()));
}
