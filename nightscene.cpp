    ...
    : QGraphicsObject(parent)
{
    ...
    m_yinxuImg = Theme::optionalImage(QStringLiteral("yinxu_projection.jpg"));
...
{
    return QRectF(886.0, 456.0, 352.0, 152.0);
}

QRectF NightBackdrop::screenRect() const
{
    // 屏幕内宽 324：要放得下「    draw(pagoda, Anyang);  // 安阳文峰塔」
    // 这样的整行中文注释，早先的 226 会把注释拦腰截断。
    return QRectF(900.0, 468.0, 324.0, 122.0);
}
...
void NightBackdrop::rebuildParticles()
{
    ...
    for (int i = 0; i < kParticleCount; ++i) {
        Particle pt;
        pt.from = QPointF(screen.center().x() + rng.range(-158.0, 158.0),
                          screen.center().y() + rng.range(-54.0, 54.0));
...
void NightBackdrop::drawPhotoBackdrop(QPainter *p) const
{
    ...
    // 等比裁剪铺满，不拉伸变形
    p->save();
    p->drawPixmap(target, m_bgImg, Theme::coverSourceRect(m_bgImg.size(), target));

    // 夜色压暗：上轻下重，底部留出更深的底座，保证叠上去的
    // 月亮 / 月饼 / 电脑 / 投影 / 粒子 与 HUD 依旧清晰可读。
    QLinearGradient veil(target.topLeft(), target.bottomLeft());
    veil.setColorAt(0.0, QColor(6, 9, 22, 104));
    veil.setColorAt(0.42, QColor(6, 9, 22, 58));
    veil.setColorAt(1.0, QColor(6, 9, 22, 176));
    p->fillRect(target, veil);

    // 顶部再压一层，让标题文字浮起来
    QLinearGradient topShade(target.topLeft(), QPointF(0.0, 132.0));
    topShade.setColorAt(0.0, QColor(4, 7, 18, 190));
    topShade.setColorAt(1.0, QColor(4, 7, 18, 0));
    p->fillRect(QRectF(0.0, 0.0, qreal(Theme::kSceneW), 132.0), topShade);
    p->restore();
}
...
void NightBackdrop::drawCampusBuilding(QPainter *p) const
{
    ...
    p->drawText(QRectF(kFacade.center().x() - 260.0, 138.0, 520.0, 18.0), Qt::AlignCenter,
                QStringLiteral("HENAN NORMAL UNIVERSITY · 留校团圆"));
...
void NightBackdrop::drawDesk(QPainter *p) const
{
    ...
    /* ---- 交互台底座 ----
     * 底图换成实拍夜景后，原先的木质书桌与照片里广场的透视对不上，
     * 所以改成一块悬在画面右下角的半透明「深夜值守台」，
     * 既把两个月饼 / 电脑从繁杂的夜景里拎出来，也不破坏实拍画面。
     */
    const QRectF deck(688.0, 428.0, 576.0, 272.0);
    QLinearGradient deckGrad(deck.topLeft(), deck.bottomLeft());
    deckGrad.setColorAt(0.0, QColor(10, 14, 28, 126));
    deckGrad.setColorAt(1.0, QColor(5, 8, 19, 190));
    p->setPen(Qt::NoPen);
    p->setBrush(deckGrad);
    p->drawRoundedRect(deck, 18.0, 18.0);
    p->setBrush(Qt::NoBrush);
    p->setPen(QPen(Theme::withAlpha(Theme::gold(), 58), 1.2));
    p->drawRoundedRect(deck, 18.0, 18.0);

    p->setFont(Theme::font(11));
    p->setPen(Theme::withAlpha(Theme::cream(), 178));
    p->drawText(QRectF(deck.left() + 18.0, deck.bottom() - 26.0, 340.0, 16.0), Qt::AlignLeft,
                QStringLiteral("深夜值守台 · 留校人的中秋夜"));

    /* ---- 月饼 ---- */
    // 盘底柔光
    QRadialGradient plateGlow(QPointF(800.0, 578.0), 116.0);
    plateGlow.setColorAt(0.0, Theme::withAlpha(Theme::gold(), 46));
    plateGlow.setColorAt(1.0, Theme::withAlpha(Theme::gold(), 0));
    p->setPen(Qt::NoPen);
    p->setBrush(plateGlow);
    p->drawEllipse(QPointF(800.0, 578.0), 116.0, 116.0);

    const QRectF cake2 = Theme::centeredRect(QPointF(730.0, 566.0), 60.0, 60.0);
    if (!m_cakeImg.isNull())
    ...
    const QRectF cake = mooncakeRect().adjusted(12.0, 12.0 - lift, -12.0, -12.0 - lift);
    if (!m_cakeImg.isNull())
    ...
    if (hoverCake) {
        p->setPen(QPen(Theme::withAlpha(Theme::gold(), 180), 2.0, Qt::DashLine));
        p->setBrush(Qt::NoBrush);
        p->drawEllipse(mooncakeRect().adjusted(-8.0, -8.0, 8.0, 8.0));
        p->setPen(Qt::NoPen);
    }
    ...
    p->drawRect(laptop.translated(0.0, -laptopLift));
    p->setBrush(QColor(58, 62, 78));
    p->drawRect(QRectF(laptop.left(), laptop.top() - laptopLift, laptop.width(), 8.0));

    const QRectF screen = screenRect().translated(0.0, -laptopLift);
    p->setBrush(QColor(9, 13, 24));
    p->drawRect(screen);

    // 屏幕里的代码：作品自己的"源代码"，也是「以代码贺华诞」的落点
    p->save();
    p->setClipRect(screen.adjusted(2.0, 2.0, -2.0, -2.0));
    p->setFont(Theme::font(11));
    const QStringList lines{
        QStringLiteral("#include <moon.h>            // 中秋"),
        QStringLiteral("#include <national_day.h>    // 国庆"),
        QStringLiteral("while (on_campus) {"),
        QStringLiteral("    send(mooncake, home);  // 小家团圆"),
        QStringLiteral("    draw(pagoda, Anyang);  // 安阳文峰塔"),
        QStringLiteral("    lit_window(me);        // 点亮留校的窗"),
        QStringLiteral("    wish(PRC, Birthday77); // 祖国华诞"),
        QStringLiteral("}")
    };
    const qreal lineH = 13.0;
    // 打字机速度：约 1.7 秒打完八行，静态截图时（2 秒）已经是一屏完整的代码
    const int visible = int(m_time * 5.0);
    for (int i = 0; i < lines.size() && i <= visible; ++i) {
        const bool last = (i == visible);
        p->setPen(Theme::withAlpha(i % 2 == 0 ? Theme::codeGreen() : Theme::gold(), last ? 240 : 175));
        p->drawText(QPointF(screen.left() + 10.0, screen.top() + 17.0 + i * lineH), lines.at(i));
    }
    if (visible < lines.size() && std::fmod(m_time * 2.0, 1.0) < 0.62) {
        const int row = qBound(0, visible + 1, lines.size() - 1);
        p->setBrush(Theme::codeGreen());
        p->drawRect(QRectF(screen.left() + 10.0,
                           screen.top() + 17.0 + row * lineH - 9.0, 6.0, 11.0));
    }
    p->restore();

    // 屏幕外溢的光
    QRadialGradient screenGlow(screen.center(), 200.0);
    screenGlow.setColorAt(0.0, Theme::withAlpha(Theme::codeGreen(), int(24.0 + 10.0 * std::sin(m_time * 2.0))));
    screenGlow.setColorAt(1.0, Theme::withAlpha(Theme::codeGreen(), 0));
    p->setBrush(screenGlow);
    p->drawRect(QRectF(screen.center() - QPointF(200.0, 200.0), QSizeF(400.0, 400.0)));

    if (hoverLaptop) {
        p->setPen(QPen(Theme::withAlpha(Theme::codeGreen(), 180), 2.0, Qt::DashLine));
        p->setBrush(Qt::NoBrush);
...
void NightBackdrop::drawProjection(QPainter *p) const
{
    ...
    // 实拍照各管一幕：安阳文峰塔 -> 「小家团圆」，殷墟博物馆夜景 -> 「甲骨·国」。
    // 校园团圆一幕以及两张实拍照都缺席时，仍走下面的矢量绘制，保证丢图也能跑。
    const QPixmap *projPhoto = nullptr;
    if (m_proj == AnyangHome && !m_projImg.isNull())
        projPhoto = &m_projImg;
    else if (m_proj == OracleGuo && !m_yinxuImg.isNull())
        projPhoto = &m_yinxuImg;

    if (projPhoto) {
        // 实景按「等比裁剪铺满」贴进圆幕，不拉伸变形
        QPainterPath clip;
        clip.addEllipse(center, radius, radius);
        const QRectF frame(center.x() - radius, center.y() - radius, radius * 2.0, radius * 2.0);
        p->save();
        p->setClipPath(clip);
        p->setOpacity(alpha * 0.98);
        // 用三参数版本（目标矩形 / 图 / 源矩形），Qt5、Qt6 都支持
        p->drawPixmap(frame, *projPhoto, Theme::coverSourceRect(projPhoto->size(), frame));
        p->restore();
        // 罩一层夜色，让照片融进整幅画面
        p->setPen(Qt::NoPen);
        p->setBrush(Theme::withAlpha(QColor(8, 12, 26), 74));
        p->drawEllipse(center, radius, radius);
        // 殷墟一幕：把甲骨文「国」压在夜景立面上，像投上去的铭文
        if (m_proj == OracleGuo) {
            paintOracleGlyph(p, QRectF(center.x() - 100.0, center.y() - 100.0, 200.0, 200.0),
                             1, Theme::withAlpha(Theme::lightGold(), 196));
        }
        // 圆幕边缘的暖色光边，呼应月亮
        QRadialGradient rim(center, radius);
        rim.setColorAt(0.0, Theme::withAlpha(Theme::gold(), 0));
        rim.setColorAt(0.80, Theme::withAlpha(Theme::gold(), 12));
        rim.setColorAt(1.0, Theme::withAlpha(Theme::lightGold(), 86));
        p->setBrush(rim);
        p->drawEllipse(center, radius, radius);
    } else {
        switch (m_proj) {
    ...
    QString title;
    switch (m_proj) {
    case AnyangHome: title = QStringLiteral("小家团圆 · 安阳文峰塔"); break;
    case OracleGuo: title = QStringLiteral("家国同庆 · 安阳殷墟甲骨"); break;
...
void NightBackdrop::drawHud(QPainter *p) const
{
    ...
    Theme::drawGlowText(p, QPointF(150.0, 44.0), Theme::workTitle(),
                        Theme::cream(), Theme::gold(), 6.0);
    p->setFont(Theme::font(12));
    p->setPen(Theme::withAlpha(Theme::cream(), 168));
    p->drawText(QRectF(24.0, 60.0, 460.0, 18.0), Qt::AlignLeft,
                QStringLiteral("%1 ｜ %2").arg(Theme::activityTheme(), Theme::schoolName()));
    ...
    auto bubble = [&](const QPointF &anchor, const QString &text, bool visible, const QColor &accent) {
        ...
        p->setBrush(Theme::withAlpha(QColor(10, 14, 28), 212));
    ...
    bubble(QPointF(1062.0, 398.0), QStringLiteral("点我 · 用代码贺华诞"),
           !m_codeRunning, Theme::codeGreen());

    // 左下操作说明
    p->setPen(Qt::NoPen);
    p->setBrush(Theme::withAlpha(QColor(6, 9, 20), 186));
    p->drawRoundedRect(QRectF(24.0, 636.0, 496.0, 58.0), 12.0, 12.0);
    p->setPen(QPen(Theme::withAlpha(Theme::gold(), 54), 1.0));
    p->setBrush(Qt::NoBrush);
    p->drawRoundedRect(QRectF(24.0, 636.0, 496.0, 58.0), 12.0, 12.0);
    p->setFont(Theme::font(12));
    p->setPen(Theme::withAlpha(Theme::cream(), 200));
    p->drawText(QRectF(40.0, 646.0, 470.0, 20.0), Qt::AlignLeft,
                QStringLiteral("拖动月亮：切换投影　点击月饼：安阳入梦　点击电脑：代码献礼"));
    p->setPen(Theme::withAlpha(Theme::cream(), 140));
    p->drawText(QRectF(40.0, 668.0, 470.0, 20.0), Qt::AlignLeft,
                QStringLiteral("献给每一位留校过双节的同学 · 2026 国庆七十七华诞 · Esc 返回首页"));
